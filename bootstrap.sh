...
if [[ "$bootstrap_github" =~ ^[Yy]$ ]]; then
    ...
    if grep -q "$GITHUB_KEY_PATH" "$SSH_CONFIG_PATH"; then
        echo "Path already added to ssh config"
    else
        printf "Host github-$GITHUB_EMAIL\n  HostName github.com\n  AddKeysToAgent yes\n  IdentityFile $GITHUB_KEY_PATH\n" >> "$SSH_CONFIG_PATH"
    ...
    # TODO CHECK THIS AND RETRY IF NOT WORKING
else
...
read -p "Enter your desired environment root directory [$DEVELOP_DIR]: " develop_dir_input
DEVELOP_DIR="$HOME/${develop_dir_input:-$DEVELOP_DIR}"
DOTFILES="$DEVELOP_DIR/dotfiles"

echo "Setting up with '$DEVELOP_DIR' as target environment root directory"
mkdir -p $DEVELOP_DIR
cd $DEVELOP_DIR
git clone git@github.com:tuckerchapin/dotfiles.git
